...
bool readSFMLController(){
    std::ifstream sfmlControlFile("../system_sfml_controller.txt");
    if(sfmlControlFile.is_open())
    {
        ...
        while(std::getline(sfmlControlFile, line))
        {
            if(line == "update"){
                // clear the file
                std::ofstream clear_file("../system_sfml_controller.txt", std::ofstream::trunc);
                clear_file.close();
                return true;
            }
        }
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed)
                ...
                current_name_text.setPosition(sf::Vector2f(0,0));
            }
        }
